...
// 'tracked' = "We're"
console.log(tracked)
// 'tracked' was declared inside the function so it was in the local scope and not the global scope
...
// 'tracked' = "We're"
console.log(tracked)
// arg is the argument to the function and it is a pointer to the value of 'tracked' when we reassign 'arg', it doesn't change the value of 'tracked'
...
function loseTracked3(tracked) {
  ...
  // 'tracked' = undefined
console.log(tracked);
// while 'tracked' is defined outside the function, 'tracked' inside the function points to the argument we didn't define when we called 'loseTracked3'
}
...
// 'tracked' = "to"
console.log(tracked);
// because 'tracked' isn't defined within the function, it will look outside the function to find 'tracked' in the global scope and assigned the new value to the global 'tracked'
...
// 'tracked' = "love"
console.log(tracked);
// 'tracked is defined locally, so the global 'tracked' is assigned the value of 'arg' argument that is "love"
...
// 'tracked' = "the"
console.log(tracked);
// the 'tracked' that's changed in 'losetracked7' is the local copy of 'tracked', since the argument was alos named 'tracked'.  So it didn't have to look globally for the variable 'tracked'.  So it didn't look globally for the variable 'tracked'
...
// 'tracked' = "the"
console.log(tracked);
// 'tracked' was declared locally and us assigning the value of the argument to 'tracked' doesn't change the fact that it didn't havt to look outside to the global scope
...
// 'tracked' = "the"
console.log(tracked);
// 'loseTracked9' is never invoked, so the code inside is never run, so the value of 'tracked' isn't changed
...
// 'tracked' = "so"
console.log(tracked);
// 'newTracked' was defined locally in 'loseTracked10', so the global 'newTracked' wasn't reassigned so when assigned value of newTracked' into 'tracked', the value of 'newTracked' was given
...
// 'tracked' = ["WHAT!?", "Arrays!"]
console.log(tracked);
// 'arg' is a pointer to the argument which is the array that 'tracked' points so when we reassign values within 'arg', it will point to the same array as 'tracked', changing the contents of the array itself will change for both variables
...
// 'tracked' = "Never Gonna Give You Up"
console.log(tracked);
// pointers to a string can also go for booleans, integers and floats